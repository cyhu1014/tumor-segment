...
txt_path = 'tumor_analysis.csv'
text_file = open(txt_path, "w")
text_file.write( 'file_name,min_x,max_x,min_y,max_y,min_z,max_z,x_size,y_size,z_size\n')
for i,(label,abs_path) in enumerate(train_loader):
    ...
    for i in range (label.shape[2]-1,-1,-1):
        if(label[0,:,i,:].sum()>0):
            max_y = i
            break
    for i in range (0,label.shape[3]):
        if(label[0,:,:,i].sum()>0):
            min_z = i
            break    
    for i in range (label.shape[3]-1,-1,-1):
        if(label[0,:,:,i].sum()>0):
            max_z = i
            break
    text_file.write('%s,%d,%d,%d,%d,%d,%d,%d,%d,%d\n'%(abs_path[0],min_x,max_x,min_y,max_y,min_z,max_z,max_x-min_x,max_y-min_y,max_z-min_z))
text_file.close()
